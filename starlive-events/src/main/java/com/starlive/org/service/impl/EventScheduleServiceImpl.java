...
public class EventScheduleServiceImpl extends ServiceImpl<EventScheduleMapper, EventSchedule>
    ...
    @Override
    public void addEventSchedule(Long event_id, List<EventScheduleDto> eventScheduleDtoList) throws IOException {
        ...
        // 2. 批量查询该活动的所有日程并将现有日程按名称存入 map，方便后续查找
        List<EventSchedule> existingSchedules = eventScheduleMapper.findByEventId(event_id);
        // 3. 将现有日程按名称存入 map，方便后续查找
        Map<String, EventSchedule> existingScheduleMap = existingSchedules.stream()
                .collect(Collectors.toMap(EventSchedule::getScheduleName, schedule -> schedule));
        // 4.循环处理每个 EventScheduleDto
        List<EventScheduleVo> eventScheduleVoList = new ArrayList<>();
        for (EventScheduleDto scheduleDto : eventScheduleDtoList) {
            EventSchedule eventSchedule = new EventSchedule();
            // 4.1将 EventScheduleDto 复制到 EventSchedule
            ...
            // 4.1 检查是否已存在相同的日程
            EventSchedule existingSchedule = existingScheduleMap.get(scheduleDto.getScheduleName());
            if (existingSchedule != null) {
                // 4.2 如果已存在相同的记录，则更新数据
                eventSchedule.setId(existingSchedule.getId()); // 设置现有记录的ID
                eventScheduleMapper.updateById(eventSchedule); // 执行更新操作
            } else {
                // 4.3 如果不存在相同的记录，则插入新记录
                eventScheduleMapper.insertSchedule(eventSchedule);
            }
            // 4.3将 EventSchedule 转换为 EventScheduleVo
        ...
        if (eventDocument != null) {
            ...
            log.debug("更新到es的活动日程:{}", mergedSchedule);
            eventDocument.setSchedule(mergedSchedule);
            // 4.5保存更新后的文档到 Elasticsearch
            try {
                eventDocumentRepository.save(eventDocument);
            } catch (Exception e) {
                String msg = e.getMessage();
                log.error("保存活动日程到es失败:{}", msg);
